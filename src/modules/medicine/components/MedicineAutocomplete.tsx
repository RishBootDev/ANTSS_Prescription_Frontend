...
const medicineSubLabel = (medicine: MedicineMaster) =>
  [medicine.genericName, medicine.dosageForm, medicine.dosage, medicine.instructions]
    .filter(Boolean)
  ...
  const handleCreated = async (medicine: MedicineMaster) => {
    ...
    setResults((current) => {
      ...
      const withoutDuplicate = current.filter(
        (item) => getMedicineId(item) !== createdId
      );
    ...
    if (medicine.medicineName) {
      try {
        const refreshed = await medicineService.searchMedicines(
        ...
        ? createPortal(
            ...
            >
              <div className="max-h-72 overflow-y-auto p-1">
                ...
                {!error &&
                  activeResults.map((medicine, index) => (
                    <button
                      key={String(
                ...
                {showNotFound ? (
                  <div className="space-y-2 px-3 py-3">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Medicine not found. Add to Master?
                    </p>
