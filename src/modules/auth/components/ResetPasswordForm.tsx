...
export default function ResetPasswordForm({ token, onSuccess, onCancel }: ResetPasswordFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response: any = await apiClient.post("/api/auth/reset-password", {
        token,
        newPassword,
        confirmPassword,
      });
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-3 pt-2">
          <Button
            type="submit"
            disabled={isSubmitting}
            className="w-full h-[56px] rounded-xl bg-gradient-to-r from-[#b30d0d] to-[#d11b1b] text-base font-medium text-white shadow-lg shadow-red-500/10 transition-all duration-300 hover:scale-[1.01] hover:from-[#9f0b0b] hover:to-[#c91515] focus-visible:ring-4 focus-visible:ring-red-100"
          >
            {isSubmitting ? (
          ...
          <Button
            ...
            onClick={onCancel}
            disabled={isSubmitting}
            className="w-full h-[54px] rounded-xl text-sm font-medium text-neutral-600 hover:text-neutral-900 hover:bg-neutral-50 transition"
          >
            Cancel
          </Button>
        </div>
