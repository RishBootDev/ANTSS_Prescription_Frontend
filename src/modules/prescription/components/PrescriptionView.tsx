...
export function convertPatientDataToPrescription(
  ...
  if (patientData.diagnoses && patientData.diagnoses.length > 0) {
    diagnosis = patientData.diagnoses
      .map((d: any) => `${d.diagnosis}${d.snomedCode ? ` (${d.snomedCode})` : ""}`)
      .join(", ");
  }
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <Card className="rx-section rounded-none border border-slate-400 shadow-none">
            <CardHeader className="px-2 py-1">
              <CardTitle className="text-sm font-semibold">Prescription (Rx)</CardTitle>
            </CardHeader>
                ...
                <table className="w-full border-collapse border border-slate-400 text-xs leading-tight">
                  <thead>
                    <tr>
                      ...
                      <th className="w-[12%] border border-slate-400 px-1 py-[3px] text-center font-semibold print:hidden">
                        Tracker
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {prescription.medicines.map((medicine, index) => (
                      <tr key={medicine.id}>
                        <td className="border border-slate-400 px-1 py-[3px] align-top">{index + 1}</td>
                        <td className="border border-slate-400 px-1 py-[3px] align-top">
                          ...
                          {medicine.brandName && medicine.brandName !== medicine.genericName && (
                            <div className="text-[10px] leading-tight text-slate-500">{medicine.brandName}</div>
                          )}
                        </td>
                        <td className="border border-slate-400 px-1 py-[3px] align-top">{medicine.dosage}</td>
                        <td className="border border-slate-400 px-1 py-[3px] align-top">
                          {formatFrequency(medicine.frequency)}
                        </td>
                        <td className="border border-slate-400 px-1 py-[3px] align-top">{medicine.duration}</td>
                        ...
                        <td className="border border-slate-400 px-1 py-[3px] print:hidden">
                          <div className="flex justify-center gap-[2px]">
                            {Array.from({ length: 7 }, (_, i) => (
                              <span key={i} className="block h-3 w-3 border border-slate-400" aria-hidden="true" />
                            ))}
                          </div>
                        </td>
                      </tr>
                ...
                {prescription.diagnostics.map((test) => (
                  <div key={test.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-2 h-2 rounded-full ${test.priority === "urgent" ? "bg-red-500" : "bg-teal-500"}`}
                      />
                      <div>
                        <p className="font-medium text-sm">{test.name}</p>
                        {test.labName && <p className="text-xs text-slate-500">{test.labName}</p>}
                      </div>
                    </div>
                    <span
                      className={`text-xs px-2 py-1 rounded ${
                        test.priority === "urgent" ? "bg-red-100 text-red-700" : "bg-teal-100 text-teal-700"
                      }`}
                    >
          ...
          {prescription.followUp && (
            <div className="bg-slate-100 p-4 rounded-lg border-l-4 border-teal-500">
              ...
              <p className="text-slate-700">
                ...
                {prescription.followUp.note && <span className="text-slate-600"> — {prescription.followUp.note}</span>}
              </p>
            </div>
          )}
        </CardContent>
